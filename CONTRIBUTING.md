...
- Vue 3（Composition API，`<script setup>`）+ Vite 5，纯静态站点，无 TypeScript、无后端
- GSAP ScrollTrigger 驱动滚动叙事；WAAPI（`el.animate`）驱动一次性补间
- 图标：Iconify Vue（`@iconify/vue`，见 `src/plugins/icons.js`）
- 托管：Cloudflare Pages（`wrangler.toml`），无 lint/格式化工具链（以本文件为准）
...
src/
  main.js           开屏 boot 编排:描边→飞行→变形→分裂,滚动锁,旧版回流幕布
  App.vue           页面骨架与分页
  components/       HeroSection / PagePager / PageSection / ThemeToggle
...
- 修过的坑要把**根因**留在注释里（例：main.js 开屏衔接方案 D 的根因记录），防止后人回退成旧方案

## JavaScript

- ESM；单引号、不加分号（与现有文件一致）
- 常量大写蛇形并带单位后缀：`MOVE_MS`、`TARGET_FS`、`SOC_FADE_STAGGER_MS`
- 跨组件共享状态放 `composables/`，用模块单例 `ref`（例：`brandDock.js` 的 `bootDone`、`useTheme.js` 的 `theme`），不要挂 `window`——已存在的 `window.__finishBoot` 等是历史兜底，新代码勿效仿
...
| 数据 | A | B | 保障 |
| --- | --- | --- | --- |
| 品牌字形 path | `src/data/brandGlyph.js` | `index.html` 内联 preloader SVG | main.js `assertBrandSync` dev 断言 |
...
- 拆分去向：跨组件状态/逻辑 → `composables/use*.js`；静态数据 → `data/`；独立 UI → `components/`；纯工具函数 → 就近新建模块
- 拆分原则按**关注点**切（一块代码回答「我管什么」只能有一个答案），不按「第几百行切一刀」
- 已知超标（历史债务，勿继续扩大）：`HeroSection.vue`（~1200 行）、`main.js`（~940 行）。对它们只做必要修改，新增能力一律另立模块

## 术语表（动效资产命名，注释/代码统一用这套）

| 术语 | 含义 | 代码 |
| --- | --- | --- |
| veil（幕布） | 开屏 loading 的全屏背景层 | `.boot-veil`（main.js） |
| bar（横梁） | 合并态的整条圆角长方形。开屏 = veil 收回态（`barRect`）；滚动 = 社交底块合并态（`.social-beam`，`useBeamMerge.js`） | — |
| pieces（假块） | bar 分裂成的 5 个图标底块（动画期假层） | `.boot-veil-piece`（main.js） |
| beamDock | 横梁钉视口后的状态/接管入口（下一页变形用） | `useBeamMerge.js` 导出 |
